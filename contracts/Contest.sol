...
contract Contest{
	...
	function getWinner() public view validState(PHASE.done) returns (Contestant memory winner) {
		Contestant memory currentWinner;
		for (uint i=1; i<=contestantsCount; i++) {
			if (contestants[i].voteCount > currentWinner.voteCount) {
